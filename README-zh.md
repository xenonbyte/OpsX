...
- **`commands/`**: 包含各平台特定的 TOML 命令定义。
  - `gemini/opsx/*.toml`: Gemini CLI 命令。
  - `claude/opsx/*.toml`: Claude Code 命令。
  - `codex/prompts/opsx-*.toml`: Codex 提示词。
- **`skills/openspec/`**: AI 技能定义的核心内容。
...
| `/openspec --version` | 查看本地技能版本和配置摘要。 | 验证平台安装是否已经切换到 v1.1.0。 |
