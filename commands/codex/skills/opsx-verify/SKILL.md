...
Use this `opsx-verify` skill for this request. The shared OpsX workflow contract is embedded below, and detailed templates live in `references/` next to this file.
...
Execution rules:
- Follow the `verify` playbook from shared OpsX guidance and its referenced files.
...
- Verify routes must run `implementation-consistency-checkpoint` for `IMPLEMENTED` changes, then report `PASS`, `WARN`, and `BLOCK` findings with `patchTargets` and a concrete `nextStep` before archive eligibility.
- Verify execution records `implementation-consistency-checkpoint` plus `PASS`/`WARN`/`BLOCK` findings and blocks downstream archive actions until unresolved `BLOCK` findings are patched.
