...
Use this `opsx-archive` skill for this request. The shared OpsX workflow contract is embedded below, and detailed templates live in `references/` next to this file.
...
Execution rules:
- Follow the `archive` playbook from shared OpsX guidance and its referenced files.
