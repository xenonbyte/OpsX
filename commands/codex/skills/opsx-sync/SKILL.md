...
Use this `opsx-sync` skill for this request. The shared OpsX workflow contract is embedded below, and detailed templates live in `references/` next to this file.
...
Execution rules:
- Follow the `sync` playbook from shared OpsX guidance and its referenced files.
...
- Sync routes must compute conservative in-memory plans first, then emit `PASS`/`WARN`/`BLOCK`; when blocked, do not write partial sync output.
- Sync execution writes only after full-plan acceptance; when findings include `BLOCK`, do not write partial sync output.
...
- Compute sync changes in memory first, report PASS/WARN/BLOCK findings, and do not write partial sync output when blocked.
