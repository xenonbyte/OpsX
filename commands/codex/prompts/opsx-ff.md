...
---
# OpsX route: Fast-forward

Use the `opsx` skill for this request.

Workflow action: `ff`
Primary workflow entry: `$opsx-* (explicit routes only)`
Explicit action route: `$opsx-ff`

Execution rules:
- Follow the `ff` playbook from the `opsx` skill and its referenced files.
...
- `spec-split-checkpoint` runs after `specs` and before `design`; `spec checkpoint` runs after `design` and before `tasks`; `task checkpoint` runs after `tasks` and before `apply`.
