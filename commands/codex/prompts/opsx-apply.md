...
2. **Read all context**:
   ...
   - Read `.openspec.yaml` — check `checkpoint` status
...
4. **Implement tasks in order**:
   ...
   - Update `checkpoint` in `.openspec.yaml` (targetTask, tasksHash, gitRef, isDirty)
   - Update `## Current Status` in `tasks.md`
...
6. **When all tasks complete**:
   ...
   - Update `stage` to `verify` in `.openspec.yaml`
   - Show completion summary
   - Suggest `/prompts:opsx-verify` to validate, then `/prompts:opsx-archive`
...
## Current Status Block (MANDATORY)

Maintain this block at the end of `tasks.md`:
```markdown
---
...
>
> **Next Action**: Continue with task <ID> - <Description>
>
> **Resume Command**: `/prompts:opsx-resume <change-name>`
```

## Phase Boundary Guard

✅ You are in APPLY phase. This is the ONLY phase where you can modify code files.
