...
- [ ] **NAME-01**: User can install the breaking release as `@xenonbyte/opsx`.
- [ ] **NAME-02**: User can invoke the CLI as `opsx` with `--help`, `--version`, `install`, `uninstall`, `check`, `doc`, `language`, `migrate`, and `status`.
- [ ] **NAME-03**: User-facing docs, package metadata, templates, generated command text, and runtime messages consistently use `OpsX`, `opsx`, and `@xenonbyte/opsx`.
