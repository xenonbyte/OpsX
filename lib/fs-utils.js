...
function writeTextAtomic(filePath, content) {
  ...
  const tempPath = createTempSiblingPath(filePath, 'tmp');
  ensureDir(directory);
  try {
...
function createTempSiblingPath(filePath, extension = 'tmp') {
  const directory = path.dirname(filePath);
  const baseName = path.basename(filePath);
  const suffix = `${process.pid}.${Date.now()}.${randomBytes(6).toString('hex')}`;
  const normalizedExtension = String(extension || 'tmp').replace(/^\.+/, '') || 'tmp';
  return path.join(directory, `.${baseName}.${suffix}.${normalizedExtension}`);
}
...
function readTextIfFile(filePath, fallback = '') {
  try {
