...
function detectRolloutProfile(text) {
  ...
  const mentioned = includesAnyKeyword(content, ROLLOUT_KEYWORDS);
...
function detectCategoryInText(text, category) {
  return includesAnyKeyword(text, CATEGORY_KEYWORDS[category] || []);
}
...
function isDocsPath(filePath = '') {
  return (
    ...
    || /^openspec\/changes\//i.test(filePath)
    || /^commands\//i.test(filePath)
    || /^skills\/openspec\//i.test(filePath)
...
function normalizeExecutionEvidence(options = {}) {
  ...
  const constraints = unique([
    ...
    ...parseExecutionConstraintSignals(groupText)
  ]);
  const referencedCommitments = {
    specs: unique(toList(explicit.specRefs)),
    design: unique(toList(explicit.designRefs))
  };
  const implementationTerms = toTokenSet([implementationSummary, changedFiles.join('\n')].join('\n'));
...
function validateCheckpointContracts(options = {}) {
  ...
  [specPass, specNegationPass, specWarn, specBlock, taskPass, taskMissingCoverage, taskGate, executionPass, executionWarn, executionBlock, legacySpecBlock, legacyTaskBlock, legacyExecutionBlock].forEach((result) => {
