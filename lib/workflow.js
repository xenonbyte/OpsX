...
function classifyTaskGroupTdd(group, evidence, config) {
  ...
  const exemptByClass = !metadata.exemptionClass && !explicitlyExempt && Boolean(tddClass) && exemptSet.has(tddClass);
  ...
  return {
    ...
    exemptionReasonMissing: Boolean(metadata.exemptionClass) && !hasValidExemptionReason,
...
function appendTddTaskCheckpointFindings(findings, groups, evidence, config) {
  ...
  if (exemptionReasonMissing.length > 0) {
    addFinding(findings, {
      ...
      message: `TDD Exemption lines must include a non-empty reason: ${exemptionReasonMissing.join(', ')}.`,
      patchTargets: ['tasks']
    });
  }
