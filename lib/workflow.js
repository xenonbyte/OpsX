const { DEFAULT_SCHEMA, SHARED_HOME_NAME } = require('./constants');
...
function isDocsPath(filePath = '') {
  ...
  if (/^commands\//.test(normalized) || normalized.startsWith(LEGACY_SKILL_DIR_PREFIX)) return false;
  return (
...
function getPrimaryWorkflowSyntax() {
  return '$opsx <request>';
}
