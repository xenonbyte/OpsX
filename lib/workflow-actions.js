...
const ACTION_FALLBACK_LINES = Object.freeze({
  ...
  onboard: Object.freeze([
    ...
    'Guide the user to run `opsx install --platform <claude|codex|gemini[,...]>` when command routes are not installed, then use `{{newRoute}}` or `{{proposeRoute}}` to initialize the workspace.'
  ]),
  status: Object.freeze([
    ...
    'Report whether the workspace exists, include drift warnings, and recommend `{{newRoute}}` or `{{proposeRoute}}` to initialize missing workspace state.'
  ]),
  resume: Object.freeze([
    ...
    'Warn on artifact hash drift, reload from disk, and do not refresh stored hashes from read-only routes.',
    'Recommend `{{newRoute}}` or `{{proposeRoute}}` when the workspace or active change is missing.'
  ])
});
