...
const MIGRATION_JOURNAL_NAME = '.opsx-migration-journal.json';

function ensureWithinMigrationBase(basePath, targetPath, label) {
  ensureWithinBase(basePath, targetPath, `${label} base`);
}
...
function validateDestinationParents(plan, move, baseDir, label) {
  ...
  ensureWithinMigrationBase(baseDir, targetParent, label);
...
function appendMoveIfSelected(plan, move, baseDir, label) {
  ensureWithinMigrationBase(baseDir, move.from, label);
  ensureWithinMigrationBase(baseDir, move.to, label);
  ensureWithinMigrationBase(baseDir, move.sourceCheckPath, label);
...
function appendCreate(plan, repoRoot, displayPath) {
  ...
  ensureWithinMigrationBase(repoRoot, targetPath, 'repo');
  plan.creates.push({
    path: targetPath,
    display: displayPath
  });
...
function collectLegacyCandidates(plan) {
  ...
  function pushCandidate(candidatePath, reason) {
    ...
    ensureWithinMigrationBase(plan.homeDir, candidatePath, 'home');
    const display = `~/${toPosixPath(path.relative(plan.homeDir, candidatePath))}`;
...
function collectMetadataMoves(plan) {
  ...
  changeNames.forEach((changeName) => {
    ...
    ensureWithinMigrationBase(plan.cwd, move.from, 'repo');
    ensureWithinMigrationBase(plan.cwd, move.to, 'repo');
    ensureWithinMigrationBase(plan.cwd, move.sourceCheckPath, 'repo');
    plan.moves.push(move);
...
function getMigrationJournalPath(cwd) {
  const resolvedCwd = path.resolve(cwd || process.cwd());
  const journalPath = path.join(resolvedCwd, MIGRATION_JOURNAL_NAME);
  ensureWithinMigrationBase(resolvedCwd, journalPath, 'repo');
  return journalPath;
}

function createMigrationJournal(plan, orderedMoves) {
  ...
  return {
    ...
    moves: orderedMoves.map((move) => ({
      id: move.id,
      scope: move.scope,
      from: move.from,
      to: move.to,
      fromDisplay: move.fromDisplay,
      toDisplay: move.toDisplay
    })),
    completedMoves: [],
    createdPaths: [],
    createdDirectories: [],
...
function writeMigrationJournal(journalPath, journal) {
  const payload = Object.assign({}, journal, {
    updatedAt: new Date().toISOString()
  });
  writeTextAtomic(journalPath, `${JSON.stringify(payload, null, 2)}\n`);
  Object.assign(journal, payload);
}

function removeMigrationJournal(journalPath) {
  if (fs.existsSync(journalPath)) fs.rmSync(journalPath, { force: true });
}

function appendJournalMove(journal, move) {
  journal.completedMoves.push({
    id: move.id,
    scope: move.scope,
    from: move.from,
    to: move.to,
...
function appendJournalCreatedPath(journal, filePath, display = '') {
  journal.createdPaths.push({
    path: path.resolve(filePath),
    display: display || path.resolve(filePath)
  });
}

function appendJournalCreatedDirectory(journal, directoryPath, display = '') {
  const resolvedPath = path.resolve(directoryPath);
  if (journal.createdDirectories.some((entry) => entry.path === resolvedPath)) return;
  journal.createdDirectories.push({
    path: resolvedPath,
    display: display || resolvedPath
  });
}

function collectMissingParentDirectories(baseDir, targetParent) {
  const resolvedBase = path.resolve(baseDir);
  const resolvedParent = path.resolve(targetParent);
  ensureWithinMigrationBase(resolvedBase, resolvedParent, 'migration parent');

  const relativeParent = path.relative(resolvedBase, resolvedParent);
  if (!relativeParent) return [];

  const missing = [];
  let current = resolvedBase;
  relativeParent.split(path.sep).filter(Boolean).forEach((segment) => {
    current = path.join(current, segment);
    if (!fs.existsSync(current)) missing.push(current);
  });
  return missing;
}

function trackCreatedParentDirectories(journal, journalPath, baseDir, targetParent) {
  const missingDirectories = collectMissingParentDirectories(baseDir, targetParent);
  missingDirectories.forEach((directoryPath) => {
    appendJournalCreatedDirectory(journal, directoryPath);
  });
  if (missingDirectories.length) writeMigrationJournal(journalPath, journal);
}

function rollbackMigrationJournal(journal, logger) {
  const errors = [];

  journal.createdPaths.slice().reverse().forEach((entry) => {
    try {
      if (fs.existsSync(entry.path)) fs.rmSync(entry.path, { force: true });
      logger.debug('migration.rollback.created-path', { path: entry.path });
    } catch (error) {
      errors.push(`remove ${entry.display || entry.path}: ${error.message}`);
    }
  });

  journal.completedMoves.slice().reverse().forEach((move) => {
    try {
      if (fs.existsSync(move.to) && !fs.existsSync(move.from)) {
        ensureDir(path.dirname(move.from));
        fs.renameSync(move.to, move.from);
        logger.debug('migration.rollback.move', { from: move.to, to: move.from });
      } else if (fs.existsSync(move.to) || fs.existsSync(move.from)) {
        errors.push(`move ${move.toDisplay}: ambiguous rollback state`);
      }
    } catch (error) {
      errors.push(`move ${move.toDisplay}: ${error.message}`);
    }
  });

  const createdDirectories = Array.isArray(journal.createdDirectories) ? journal.createdDirectories : [];
  createdDirectories.slice().reverse().forEach((entry) => {
    try {
      if (fs.existsSync(entry.path)) fs.rmdirSync(entry.path);
      logger.debug('migration.rollback.created-directory', { path: entry.path });
    } catch (error) {
      errors.push(`remove empty directory ${entry.display || entry.path}: ${error.message}`);
    }
  });
...
function runMigration(options = {}) {
  ...
  const journalPath = getMigrationJournalPath(cwd);
  if (fs.existsSync(journalPath)) {
    throw new Error(`Migration aborted: previous migration journal exists at ${journalPath}`);
  }

  const orderedMoves = sortMoves(plan.moves);
  const journal = createMigrationJournal(plan, orderedMoves);
  writeMigrationJournal(journalPath, journal);
  logger.info('migration.start', {
    cwd,
    homeDir,
    moves: orderedMoves.length,
  ...
  try {
    orderedMoves.forEach((move) => {
      ensureWithinMigrationBase(move.scope === 'home' ? homeDir : cwd, move.from, move.scope);
      ensureWithinMigrationBase(move.scope === 'home' ? homeDir : cwd, move.to, move.scope);
      if (!fs.existsSync(move.from)) {
        throw new Error(`Migration aborted: Planned source missing after selection: ${move.fromDisplay}`);
      }
      trackCreatedParentDirectories(journal, journalPath, move.scope === 'home' ? homeDir : cwd, path.dirname(move.to));
      ensureDir(path.dirname(move.to));
      fs.renameSync(move.from, move.to);
      moved.push(move);
      appendJournalMove(journal, move);
      writeMigrationJournal(journalPath, journal);
      logger.debug('migration.move', { from: move.fromDisplay, to: move.toDisplay });
    });

    trackCreatedParentDirectories(journal, journalPath, cwd, path.dirname(getCanonicalActivePath(cwd)));
    const activeResult = writeActiveStateIfMissing(cwd, options.activeChange || '');
    if (activeResult.created) {
      createdCount += 1;
      appendJournalCreatedPath(journal, activeResult.path, '.opsx/active.yaml');
      writeMigrationJournal(journalPath, journal);
    }
    ...
    if (isDirectoryPath(canonicalChangesDir)) {
      fs.readdirSync(canonicalChangesDir, { withFileTypes: true })
        ...
        .forEach((entry) => {
          ...
          scaffoldResult.created.forEach((createdPath) => {
            createdCount += 1;
            appendJournalCreatedPath(journal, createdPath);
          });
          if (scaffoldResult.created.length) writeMigrationJournal(journalPath, journal);
        });
    }
    ...
    logger.info('migration.complete', { moves: moved.length, creates: createdCount });
  } catch (error) {
    ...
    journal.error = error && error.message ? error.message : String(error);
    try {
      writeMigrationJournal(journalPath, journal);
    } catch (journalError) {
      logger.warn('migration.journal-write-failed', { error: journalError.message });
    }

    const rollbackErrors = rollbackMigrationJournal(journal, logger);
    if (rollbackErrors.length) {
      journal.status = 'failed';
      journal.rollbackErrors = rollbackErrors;
      writeMigrationJournal(journalPath, journal);
      throw new Error(`${journal.error} Rollback incomplete; inspect ${journalPath}: ${rollbackErrors.join('; ')}`);
    }
