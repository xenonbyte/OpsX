...
function applySyncPlan(plan) {
  ...
  const canonicalSpecsDir = normalizedPlan.canonicalSpecsDir
    ? path.resolve(normalizedPlan.canonicalSpecsDir)
    : path.resolve(normalizedPlan.repoRoot || process.cwd(), '.opsx', 'specs');
  const writes = (Array.isArray(normalizedPlan.writes) ? normalizedPlan.writes : []).map((entry, index) => {
    ...
    return {
      ...
      backupPath: path.join(path.dirname(targetPath), `.${path.basename(targetPath)}.${process.pid}.${Date.now()}.${index}.bak`),
