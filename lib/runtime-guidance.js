...
function deriveArtifactStates(options = {}) {
  ...
  const securityReviewActive = review.active !== false;
...
function buildRuntimeKernel(options = {}) {
  ...
  const sources = collectArtifactSources(runtime.changeDir);
...
function buildApplyInstructions(options = {}) {
  ...
  const tasksPath = path.join(kernel.paths.changeDir, 'tasks.md');
  ...
  const sources = collectArtifactSources(kernel.paths.changeDir);
