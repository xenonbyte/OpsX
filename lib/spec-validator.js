...
const STOPWORDS = new Set([
  ...
  'within', 'using', 'used', 'use', 'need', 'needs', 'needed', 'ensure', 'ensures', 'ensured', 'support', 'supports',
  'supported', 'add', 'adds', 'added', 'update', 'updates', 'updated', 'implement', 'implements', 'implemented',
  'define', 'defines', 'defined', 'create', 'creates', 'created', 'task', 'tasks', 'group', 'groups', 'item', 'items',
  'work', 'works', 'workflow', 'change', 'changes', 'during', 'all', 'any', 'each', 'every', 'both', 'either',
  'neither', 'todo', 'done', 'given', 'true', 'false', 'system'
]);
...
function parseProposalScope(proposalText = '') {
  ...
  lines.forEach((line) => {
    ...
    const inWhatChanges = activeSection === 'what changes';
    const inCapabilities = activeSection === 'capabilities';
    const inCapabilitySubsection = activeSubsection === 'new capabilities' || activeSubsection === 'modified capabilities';
