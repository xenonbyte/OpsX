...
function showHelp(language) {
  return formatMessage('cli.help', language, {
    productName: PRODUCT_NAME,
    version: PACKAGE_VERSION,
    productLongName: PRODUCT_LONG_NAME,
    short: PRODUCT_SHORT_NAME
  });
}
...
function showStatus(options = {}) {
  ...
  if (!fs.existsSync(canonicalProjectConfigPath)) {
    return [
      ...
      'Next: use `$opsx-new` / `/opsx-new` or `$opsx-propose` / `/opsx-propose` to initialize and select a change.',
      '',
      ...migrationLines
    ].join('\n');
  }
...
async function runCli(argv) {
  ...
  if (argv.includes('--help') || argv.includes('-h')) {
    console.log(showHelp(options.language));
    return;
  }
  ...
  switch (command) {
    case 'install': {
      ...
      results.forEach((result) => {
        console.log(formatMessage('install.success', options.language, {
          path: result.platformCommandsDir
        }));
      });
      return;
    }
    case 'uninstall': {
      ...
      console.log(removed.length
        ? formatMessage('uninstall.removed', options.language, { platforms: removed.join(', ') })
        : formatMessage('uninstall.none', options.language));
      return;
    }
    case 'check':
    ...
    case 'status':
      if (options.json === true) {
        const envelope = buildStatusJsonEnvelope({
          cwd: process.cwd(),
          homeDir: process.env.HOME
        });
        logger.info('cli.status.json', {
          cwd: process.cwd(),
          initialized: envelope.workspace.initialized === true,
          activeChange: envelope.activeChange || null,
